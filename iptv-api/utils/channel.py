...
def _get_total_urls_cached(
    ...
    urls_limit_key = config.jmtv_client_urls_limit if config.open_jmtv_client_direct_mode else config.urls_limit
    cache_key = (
        ...
        rtmp_key,
        bool(apply_limit),
        config.open_jmtv_client_direct_mode,
        urls_limit_key,
    )
        ...
        for name, old_info_list in channel_obj.items():
            ...
            for method, result in total_result:
                if config.open_method[method]:
                    ...
                    method_name_key = f"name.{method}"
                    print(f"{t(method_name_key)}:", len(name_results), end=", ")
...
async def test_speed(data, ipv6=False, callback=None, on_task_complete=None):
    ...
    urls_limit = config.jmtv_client_urls_limit if config.open_jmtv_client_direct_mode else config.urls_limit
    if urls_limit <= 0:
        urls_limit = float("inf")
...
def generate_channel_statistic(logger, cate, name, values):
    ...
    avg_fps_text = f"{avg_fps:.2f}" if avg_fps is not None else t('name.unknown')
    if config.open_full_speed_test:
        content = f"{t('name.category')}: {cate}, {t('name.name')}: {name}, {t('name.total')}: {total}, {t('name.valid')}: {valid}, {t('name.valid_percent')}: {valid_rate:.2f}%, IPv4: {ipv4_count}, IPv6: {ipv6_count}, {t('name.min_delay')}: {min_delay} ms, {t('name.max_speed')}: {max_speed:.2f} M/s, {t('name.average_speed')}: {avg_speed:.2f} M/s, {t('name.max_resolution')}: {max_resolution}, {t('name.avg_fps')}: {avg_fps_text}, {t('name.video_codec')}: {most_video_str}, {t('name.audio_codec')}: {most_audio_str}"
        logger.info(content)
        print(f"📊 {content}")
    else:
        content = f"{t('name.category')}: {cate}, {t('name.name')}: {name}, {t('name.valid')}: {valid}, IPv4: {ipv4_count}, IPv6: {ipv6_count}, {t('name.min_delay')}: {min_delay} ms, {t('name.max_speed')}: {max_speed:.2f} M/s, {t('name.average_speed')}: {avg_speed:.2f} M/s, {t('name.max_resolution')}: {max_resolution}, {t('name.avg_fps')}: {avg_fps_text}, {t('name.video_codec')}: {most_video_str}, {t('name.audio_codec')}: {most_audio_str}"
        logger.info(content)
        print(f"📊 {content}")
...
def process_write_content(
    ...
    for cate, channel_obj in data.items():
        ...
        content += f"{category_prefix}{cate},#genre#"
        first_cate = False
