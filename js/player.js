...
let switchResourceModalToken = 0;
...
function initializePageContent() {
    ...
    // 设置页面标题
    document.title = currentVideoTitle + ' - JMTV播放器';
...
async function showSwitchResourceModal() {
    const modalToken = ++switchResourceModalToken;
    const urlParams = new URLSearchParams(window.location.search);
    ...
    modalContent.innerHTML = `
        <div id="switchResourceStatus" style="text-align:center;padding:16px;color:#aaa;grid-column:1/-1;">正在搜索资源...</div>
        <div id="switchResourceGrid" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 p-4"></div>
    `;
    const status = document.getElementById('switchResourceStatus');
    const grid = document.getElementById('switchResourceGrid');
    let searchedCount = 0;
    let foundCount = 0;
    const resourceCards = [];

    const renderResourceCard = (sourceKey, result, sourceName) => {
        const isCurrentSource = String(sourceKey) === String(currentSourceCode) && String(result.vod_id) === String(currentVideoId);
        const cardId = `switch-resource-${String(sourceKey).replace(/[^\w-]/g, '_')}`;
        return `
            <div class="relative group ${isCurrentSource ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:scale-105 transition-transform'}" 
                ...
                <div class="aspect-[2/3] rounded-lg overflow-hidden bg-gray-800 relative">
                    ...
                    <!-- 速率显示在图片右上角 -->
                    <div id="${cardId}-speed" class="absolute top-1 right-1 speed-badge bg-black bg-opacity-75">
                        <span class="speed-indicator medium">测速中</span>
                    </div>
                </div>
                <div class="mt-2">
                    ...
                    <div id="${cardId}-episodes" class="text-[10px] text-gray-500 mt-1"></div>
                </div>
                ${isCurrentSource ? `
    ...
    const sortResourceCardsBySpeed = () => {
        if (!grid) return;
        resourceCards
            .slice()
            .sort((left, right) => {
                const leftSpeed = left.speedResult?.speed;
                const rightSpeed = right.speedResult?.speed;
                const leftGroup = typeof leftSpeed !== 'number' ? 1 : (leftSpeed >= 0 ? 0 : 2);
                const rightGroup = typeof rightSpeed !== 'number' ? 1 : (rightSpeed >= 0 ? 0 : 2);
                if (leftGroup !== rightGroup) return leftGroup - rightGroup;
                if (leftGroup === 0 && leftSpeed !== rightSpeed) return leftSpeed - rightSpeed;
                return left.foundIndex - right.foundIndex;
            })
            .forEach((card) => {
                if (card.element) {
    ...
    await Promise.allSettled(resourceOptions.map(async (opt) => {
        try {
            ...
            if (modalToken !== switchResourceModalToken) {
                return;
            }
            searchedCount += 1;
            if (!Array.isArray(queryResult) || queryResult.length === 0) {
                if (status) status.textContent = `已搜索 ${searchedCount}/${resourceOptions.length} 个资源，已找到 ${foundCount} 个`;
                return;
            }
            ...
            foundCount += 1;
            if (grid) {
                grid.insertAdjacentHTML('beforeend', renderResourceCard(opt.key, result, opt.name));
                const cardId = `switch-resource-${String(opt.key).replace(/[^\w-]/g, '_')}`;
                resourceCards.push({
                    key: opt.key,
                ...
                sortResourceCardsBySpeed();
            }
            if (status) status.textContent = `已搜索 ${searchedCount}/${resourceOptions.length} 个资源，已找到 ${foundCount} 个`;

            const speedResult = await testVideoSourceSpeed(opt.key, result.vod_id);
            if (modalToken !== switchResourceModalToken) {
                return;
            }
            const cardId = `switch-resource-${String(opt.key).replace(/[^\w-]/g, '_')}`;
            const speedBadge = document.getElementById(`${cardId}-speed`);
            const episodesLabel = document.getElementById(`${cardId}-episodes`);
            if (speedBadge) {
                speedBadge.innerHTML = formatSpeedDisplay(speedResult);
            }
            if (episodesLabel && speedResult.episodes) {
                episodesLabel.textContent = `${speedResult.episodes}集`;
            }
            const cardInfo = resourceCards.find((card) => card.key === opt.key);
            if (cardInfo) {
                cardInfo.speedResult = speedResult;
                sortResourceCardsBySpeed();
            }
        } catch (error) {
            if (modalToken !== switchResourceModalToken) {
            ...
            searchedCount += 1;
            console.warn(`切换资源搜索失败: ${opt.name}`, error);
            if (status) status.textContent = `已搜索 ${searchedCount}/${resourceOptions.length} 个资源，已找到 ${foundCount} 个`;
        }
    }));

    if (status && modalToken === switchResourceModalToken) {
        status.textContent = foundCount > 0 ? `资源搜索完成，共找到 ${foundCount} 个` : '未找到可切换资源';
    }
}
