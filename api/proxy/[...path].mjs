...
async function fetchContentWithType(targetUrl, requestHeaders) {
    ...
    const headers = {
        ...
        'Accept': requestHeaders['accept'] || 'image/avif,image/webp,image/apng,image/svg+xml,image/*,*/*;q=0.8', // 传递原始 Accept 头（如果有）
    ...
    try {
        ...
        const content = isBinaryContentType(contentType) ? Buffer.from(await response.arrayBuffer()) : await response.text();
