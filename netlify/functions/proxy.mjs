...
async function fetchContentWithType(targetUrl, requestHeaders) {
    ...
    try {
        ...
        const content = isBinaryContentType(contentType) ? Buffer.from(await response.arrayBuffer()).toString('base64') : await response.text();
        logDebug(`Fetch success: ${targetUrl}, Content-Type: ${contentType}, Length: ${content.length}`);
        return { content, contentType, responseHeaders: response.headers, isBase64Encoded: isBinaryContentType(contentType) };
    } catch (error) {
...
function isBinaryContentType(contentType) {
    ...
    return type.startsWith('image/') || type.startsWith('video/') || type.startsWith('audio/') || type.includes('application/octet-stream');
}
...
export const handler = async (event, context) => {
    ...
    try {
        ...
        const { content, contentType, responseHeaders, isBase64Encoded } = await fetchContentWithType(targetUrl, event.headers);
